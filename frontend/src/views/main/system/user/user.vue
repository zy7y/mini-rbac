    ...
    if (!isQuery.value) {
        getUsers({ offset, limit }).then((res) => {
            ...
            const users = res.data.items.map((item) => {
                const limit_start = item.limit_start
                    ? dayjs(item.limit_start).format('HH:mm')
                    : ''
                const limit_end = item.limit_start
                    ? dayjs(item.limit_end).format('HH:mm')
                    : ''
                return {
                    limit_time: `${limit_start} - ${limit_end}`,
                    ...item,
                }
            })
            console.log('dataSource is', users)
...
<template>
    <div class="user">
        ...
        <UserModal
            ref="modalRef"
            :modal-title="modalConf.title"
            :modal-type="modalConf.type"
        />
    </div>
</template>
